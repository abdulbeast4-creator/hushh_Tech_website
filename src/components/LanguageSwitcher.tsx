...
const LanguageSwitcher: React.FC<LanguageSwitcherProps> = ({ variant = 'light' }) => {
  ...
  // Gate all DOM writes behind a mount flag — prevents SSR hydration mismatch
  useEffect(() => {
    setMounted(true);
  }, []);

  // Reactively sync document dir + lang whenever i18n.language changes,
  // but only after the component has mounted on the client
  useEffect(() => {
    if (!mounted) return;
